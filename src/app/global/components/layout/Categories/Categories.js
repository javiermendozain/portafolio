...
class Categories extends React.Component {
  ...
  componentDidMount() {
    const {
      getAllCategories,
      getAllTag,
      categoryReducer: { categories = [], tags = [] }
    } = this.props;
    ...
    const _showCategories = () => {
      ...
      res.push(
        <div
          ...
          onClick={() =>
            this.setState({ selectedCategory: 0 }, () => history.push("/blog/"))
          }
        >
          Todo
      ...
      categories.map((item, index) => {
        res.push(
          ...
          >
            ...
            {/* Used login Omited with || pass to && to apply */}
            {!isEmpty(uid) || (
              <div
    ...
    return (
      <React.Fragment>
        <div className={style.categories}>
          <div className={style.containerItemCategory}>{_showCategories()}</div>
          {/* Used login Omited with || pass to && to apply */}
          ...
          {!isEmpty(uid) || (
            ...
            >
              ...
              <DropdownMenu>
                ...
                <DropdownItem onClick={() => this.openCloseModalAddCategory()}>
                  Categorías
                </DropdownItem>
                {/* <DropdownItem divider /> */}
                <DropdownItem onClick={() => this.openCloseModalAddTag()}>
                  Etiqueta
                </DropdownItem>
                <DropdownItem>
        ...
        <Modal
          ...
          fade
        >
          <ModalHeader toggle={() => this.openCloseModalAddCategory()}>
            Crear una categoría
          </ModalHeader>
          <ModalBody>
            <React.Fragment>
              ...
              <Input
                onChange={e => typingCategory(e.target.value)}
                defaultValue={newCategory}
                maxLength="100"
              />
              <div
                className={style.itemCategory}
                style={{ background: `${color}`, width: "20px" }}
              >
                {color}
              </div>
              <Button onClick={e => editingColorCategory(getRandomColor())}>
                {" "}
                Generar color{" "}
              </Button>
              &nbsp;
              <Button
                outline
                onClick={() =>
                  saveCategory(() => this.openCloseModalAddCategory())
                }
              >
              ...
              {!isEmpty(editCategory) && (
                <React.Fragment>
                  ...
                  <Button
                    outline
                    onClick={() =>
                      saveEditCategory(() => this.openCloseModalAddCategory())
                    }
                  >
                    <FontAwesomeIcon icon={faSave} />
                  </Button>
        ...
        <Modal
          ...
          fade
        >
          <ModalHeader toggle={() => this.openCloseModalAddTag()}>
            Crear una etiqueta
          </ModalHeader>
          <ModalBody>
            <Label> Nombre de etiqueta</Label>
            <Input
              onChange={e => editingTag(e.target.value)}
              defaultValue={newTag}
              maxLength="100"
            />
          </ModalBody>
...
const mapStateToProps = ({ categoryReducer, firebase }) => ({
  categoryReducer,
  firebase
});
